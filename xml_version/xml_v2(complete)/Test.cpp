...
int main(int argc, char* argv[])
{
 ...
 //   char* data = NULL;
  ...
  //  char * inputpath = "C:\\Users\\User\\Test5.xml";

    char * inputpath = "firstTest62.xml";

  //  readData(argv[1], Alva_File, &data, &length);
    
    
    char data[9999] = {0};
    ifstream file1(inputpath);
    if (!file1.is_open()) {
        exit(1);
    }
    else {
        file1.read(data, 9999);
        file1.close();
    }
    ...
    pXmlElement current = xml->body;
    cout << "xml->body's name is " << xml->body->name << endl;

    while (current)
    {
        if (!strncmp(current->name, "annotation", strlen("annotation")))
        {
            pXmlElement Sec_current = current->content->pelement;
            while(Sec_current)
            {
                
                if (!strncmp(Sec_current->name, "object", strlen("object")))
                {
                    pXmlElement Third_current = Sec_current->content->pelement;
                    while (Third_current)
                    {
                    
                        if (!strncmp(Third_current->name, "bndbox", strlen("bndbox")))
                        {
                            pXmlElement fourth_current = Third_current->content->pelement;
                            while (fourth_current)
                            {
                                if (!strncmp(fourth_current->name, "xmin", strlen("xmin")))
                                {
                                    cout << "xmin value is  " << fourth_current->content->value << endl;
                                }
                                if (!strncmp(fourth_current->name, "ymin", strlen("ymin")))
                                {
                                    cout << "ymin value is  " << fourth_current->content->value << endl;
                                }
                                if (!strncmp(fourth_current->name, "xmax", strlen("xmax")))
                                {
                                    cout << "xmax value is  " << fourth_current->content->value << endl;
                                }
                                if (!strncmp(fourth_current->name, "ymax", strlen("ymax")))
                                {
                                    cout << "ymax value is  " << fourth_current->content->value << endl;
                                }
                                fourth_current = fourth_current->next;
                            }
                ...
                /*
                if (!strncmp(Sec_current->name, "train", strlen("Train")))
                {
                    cout << "label of Train's value is " << Sec_current->content->value << endl;
                }
                */
    ...
    FILE* file = NULL;
    fopen_s(&file, "firstTest15.xml", "wb");
    fileXml(xml, file);
    fclose(file);

    freeXml(xml);

    pXmlElement xmlElementRoot;
    
/*
    pXmlElement xmlElementRoot = (pXmlElement)malloc(sizeof(XmlElement));
    memset(xmlElementRoot, 0, sizeof(XmlElement));
    char * ptr = findElementsbyname(data,xmlElementRoot,"annotation");
    ptr = findElementsbyname(ptr, xmlElementRoot, "folder");
    */
