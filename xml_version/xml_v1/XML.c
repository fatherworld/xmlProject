...
#define FUNC_CODE 0x01
    ...
    /*
    char * pD = data;
    char* hasContValue1 = strstr(data, "<");
    char* hasContValue2 = strstr(data, "/");
    if (hasContValue1)
    {
        if (hasContValue2)
...
#define FUNC_CODE 0x04
    ...
    char temp_elename[30] = { 0 };
    xmlElement->name = getName(eleStart);
    char* nameEnd = strstr(eleStart, ">");
    strncpy(temp_elename, eleStart, nameEnd - eleStart);
    ...
    /*
        char* otherbegin = strstr(data, "/");
        char* otherend = strstr(data, ">");
        if()

        memcpy(temp_elename, otherbegin, otherend - otherbegin);
        if (strncmp(temp_elename, xmlElement->name, strlen(xmlElement->name)))
        {
            eleStart = otherend;
        }
        */
...
static char* findContent(char* data, char* end, pXmlContent* xmlContent) {
    ...
    if (content_is_value(ptr, end))
    {
        ...
        ((*xmlContent)->value) = (char*)malloc(sizeof(strstr(ptr, "</") - ptr));
        memset((*xmlContent)->value, 0, strstr(ptr, "</") - ptr);
